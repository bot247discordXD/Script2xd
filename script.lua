...
queueonteleport("https://raw.githubusercontent.com/bot247discordXD/ScriptsThiago/main/ScriptsPlsDonate.lua'))()")
...
--GUI
local Window = library:AddWindow("PornHUB",
{
...
webhookTab:AddButton("Mensaje para conseguir bobuts", function()
    if getgenv().settings.webhookBox then
        webhook("Agradeceriamos si puedes donar : https://www.roblox.com/users/570260666/profile @everyone")
    end
end)
...
Players.LocalPlayer.leaderstats.Raised.Changed:Connect(function()
    ...
    if getgenv().settings.webhookToggle and getgenv().settings.webhookBox then
        ...
        if string.find(logs[#logs].message, Players.LocalPlayer.DisplayName) then
            webhook(tostring(logs[#logs].message.. " (Total: ".. Players.LocalPlayer.leaderstats.Raised.value.. ") @everyone"))
        else
            webhook(tostring("💰 Alguien dio una propina ".. Players.LocalPlayer.leaderstats.Raised.value - RaisedC.. " Robux a ".. Players.LocalPlayer.DisplayName.. " (Total: " .. Players.LocalPlayer.leaderstats.Raised.value.. ")"))
        end
    end
